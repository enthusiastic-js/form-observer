...
If you read _[Why Doesn't `FormValidityObserver.configure` Require a `form` to Be Observed?](#why-doesnt-formvalidityobserverconfigure-require-a-form-to-be-observed)_, then you're aware that the `configure` method of the `FormValidityObserver` can be enhanced to _automatically_ apply the proper attributes to a configured field based on the constraints that are supplied in the method call. Although this _is_ done in our framework-specific implementations, it is _not_ done in the core class. Some of you (including myself) may wonder why.
