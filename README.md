...
- **No Dependencies**: The `Form Observer` packs _a lot_ of power into a _tiny_ bundle to give your users the best experience. The **entire `@form-observer/core` library** is only 4.32kb gzipped (without minification) when all comments are removed (JSDocs, etc.). (If you choose to use a [JS Framework Integration](./docs/form-validity-observer/integrations/README.md), then the _total_ size is 5kb gzipped under the same circumstances.)
